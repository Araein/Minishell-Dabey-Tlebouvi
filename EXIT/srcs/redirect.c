...
/*   Updated: 2023/01/22 15:49:17 by tlebouvi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	setup_out(int fd[2], char *doc, int type)
{
	...
	return (1);//pas oublier de changer erreur
}
...
int	redirect(t_list *list)
{
	...
	while (list->cmd[i])
	{
		...
		// 	//list->to_unlink = list->cmd[i + 1];
		// }
		if (ft_strcmp(list->cmd[i], "<") == 0 || ft_strcmp(list->cmd[i], "<<") == 0)
			setup_in(list->redirfd, list->to_unlink, list->cmd[i + 1], 0);
		else if (ft_strcmp(list->cmd[i], ">>") == 0)
			setup_out(list->redirfd, list->cmd[i + 1], 1);
		else if (ft_strcmp(list->cmd[i], ">") == 0)
			setup_out(list->redirfd, list->cmd[i + 1], 0);
		i++;
