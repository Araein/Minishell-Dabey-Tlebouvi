...
/*   Updated: 2022/12/23 16:12:06 by tlebouvi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	dupbonus(t_pipe *pipex, int pipefd[2], int pipeout[2], int type)
{
	...
	{
		close (pipefd[0]);
		fdup2(pipefd[1], 1); //d de sortie (piped pipe)
		close(pipefd[1]);
		dup2(pipex->fd[0], 0); //fd d'entree (document)
		close(pipex->fd[0]);
	}
	if (type == 2)
	{
		close(pipefd[1]);
		dup2(pipefd[0], 0); //fd d'entree (piped pipe)
		close(pipefd[0]);
		dup2(pipex->fd[1], 1); //fd de sortie (document)
		close(pipex->fd[1]);
	}
	if (type == 3) //pipeout et pipefd sont piped
	{
		close(pipefd[1]); 
		dup2(pipefd[0], 0); // -> on va lire a partir de pipefd[0]
		close(pipefd[0]);
