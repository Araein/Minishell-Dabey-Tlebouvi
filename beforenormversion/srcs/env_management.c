...
// Minishell$> rwegreg
// rwegreg: command not found



/*

pour fire les retour d'erreur -> 

le mettre a 0 -> quand echo $? est call




Minishell$>  << stop exit > outfile
> coucou
> stop
...
exit ergerthtr
exit 257 -> et n'importe auel chiffre



egoherog | orgr | oerghoerhg

-> messages d'erreur overlap  (le fait probablement pour tous le smessages d'erreur)
cat erghrthrt

gerer les erreurs de file avec les redirections, gerer les erreurs de pid, gerer les errerus d'exec



gerer le retour de pipe si erreur

gerer tous les retours de malloc */
